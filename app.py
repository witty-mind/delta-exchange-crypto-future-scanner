...
import os
import requests, datetime, time, threading, logging, sqlite3
...
def ohlc_refresh_loop():
    while True:
        try:
            ...
            if "interpreter shutdown" in str(e): return
            log.error("OHLC RuntimeError: %s", e)
...
def ema_refresh_loop():
    while True:
        try:
            ...
            if "interpreter shutdown" in str(e): return
            log.error("EMA RuntimeError: %s", e)
        ...
        try:
            ...
            if ticks:
                ...
                if cache.ema_ready:
                    ...
                    full_payload = {
                        "market_breadth": cache.get_market_breadth(),
                        "futures": cache.get_futures_table(),
                        "above_pdh": a_pdh,
                        "below_pdl": b_pdl,
                        "above_ema": ema_data,
                        "ema_pullback_buy": pb_buy_data,
                        "ema_pullback_sell": pb_sell_data,
                        "pdh_ema_confluence": conf_data,
                        "ichimoku_stack": ich_data,
                        "ichimoku_bear": bear_data,
                        "ichimoku_classic_bull": classic_bull_data,
                        "ichimoku_classic_bear": classic_bear_data
                    }
                    socketio.emit("scan_results", full_payload)
                    first_run = False
...
if __name__ == "__main__":
    import subprocess
    try:
        # Auto-kill zombie processes on port 5001 to prevent 'Address already in use' errors
        subprocess.run("lsof -ti:5001 | xargs kill -9", shell=True, stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
        time.sleep(0.5)
    except Exception:
        pass
